...
class MainActivity extends AppCompatActivity with EventBus.RefOwner
  ...
  lazy val mainLayout = c[FrameLayout](IO(drawer)(
    ...
    IO(drawerLeft)(
      IO(channels) >>= lp(MATCH_PARENT, MATCH_PARENT) >>= listTweaks >>= kitkatPadding(true, false)
    ) >>=
    ...
    IO(drawerRight)(
      ...
      IO(nickList) >>= lp(MATCH_PARENT, MATCH_PARENT) >>= listTweaks >>= kitkatPadding(false, true)
    ) >>= vertical >>=
